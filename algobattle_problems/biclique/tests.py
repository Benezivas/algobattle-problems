...
from pydantic import ValidationError as PydanticValidatonError
...
class Tests(unittest.TestCase):
    ...
    def test_vertices_exist(self):
        ...
        with self.assertRaises(PydanticValidatonError):
            Solution.create_and_validate({"s_1": set(), "s_2": {20}}, instance=graph)
    ...
    def test_edges_exist(self):
        ...
        with self.assertRaises(ValidationError):
            Solution.create_and_validate({"s_1": {1}, "s_2": {2}}, instance=graph)

    def test_edges_missing(self):
        """Asserts that solutions that aren't bipartite are not allowed."""
        graph = UndirectedGraph(num_vertices=10, edges=[(i, j) for i in range(10) for j in range(i)])
        with self.assertRaises(ValidationError):
            Solution.create_and_validate({"s_1": {1, 2}, "s_2": {3, 4}}, instance=graph)
